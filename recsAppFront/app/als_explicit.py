...
class ExplicitALS:
        ...
        for iteration in range(self.n_iterations):
            for u in range(n_users):
                if len(user_items[u]) > 0:
                    ...
                    A = item_factors_u.T @ item_factors_u + self.user_reg * np.eye(
                        self.n_factors
                    )
            ...
            for i in range(n_items):
                if len(item_users[i]) > 0:
                    ...
                    A = user_factors_i.T @ user_factors_i + self.item_reg * np.eye(
                        self.n_factors
                    )
            ...
            if validation_matrix is not None:
                ...
                self.training_history.append(
                    {"iteration": iteration, "train_mse": train_mse, "val_mse": val_mse}
                )
                ...
                if iteration % 2 == 0 or iteration == self.n_iterations - 1:
                    print(
                        f"Iteration {iteration}, Train MSE: {train_mse:.4f}, Val MSE: {val_mse:.4f}"
                    )
                ...
                self.training_history.append(
                    {"iteration": iteration, "train_mse": train_mse}
                )
    ...
    def recommend_for_new_user(self, user_ratings, n_recommendations=10):
        """
        Get top N recommendations for a new user based on their ratings.

        Args:
            user_ratings: Dict mapping item_indices to ratings
            n_recommendations: Number of recommendations to return

        Returns:
            List of (item_index, predicted_rating) tuples
        """
        rated_item_indices = np.array(list(user_ratings.keys()))
        user_ratings_values = np.array(list(user_ratings.values()), dtype=float)

        if len(rated_item_indices) == 0:
            return []

        rated_items_factors = self.item_factors[rated_item_indices]

        A = rated_items_factors.T @ rated_items_factors + self.user_reg * np.eye(
            self.n_factors
        )
        b = rated_items_factors.T @ user_ratings_values
